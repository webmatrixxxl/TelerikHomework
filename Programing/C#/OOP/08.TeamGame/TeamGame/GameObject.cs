...
namespace TeamGame
{
    public abstract class GameObject : ICollidable, IRenderable, IObjectProducer
    {
        ...
        {
            get { return new MatrixCoords(topLeft.Row, topLeft.Col); }
        ...
        private char[,] CopyBodyMatrix(char[,] bodyMatrixToCopy)
        {
            int rows = bodyMatrixToCopy.GetLength(0);
            int cols = bodyMatrixToCopy.GetLength(1);

            char[,] newBodyMatrix = new char[rows, cols];

            for (int row = 0; row < rows; row++)
            {
                for (int col = 0; col < cols; col++)
                {
                    newBodyMatrix[row, col] = bodyMatrixToCopy[row, col];
                }
            }
